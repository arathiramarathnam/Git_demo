...
//	System.out.println("give me conflict here to  in Sri branch");
	...
	/*
	 ...
	 java project --> push this project to github
	 
	 right repositery.
	 git add--> trouble in git code.
	 
	 you will go tot that project in cmd
	 then inside project 
	 git init -->
	 git add -->
	 git commit -->
	 
	 create git repositery in github
	 then you will take path of https for that repositery
	 
	 git remote add origin <path of the repo>
	 
	 if required you have to set username and email of the github
	 ...
	 git merge <brancheName_1> --> merge with the branch where we are
	 ...
	 //resolving conflict
	  ...
	  //Currently you are on branch b1
	  // git pull origin master  --> b1+master
	 //Currently you are on branch b1
	  //git merge <master>  --> b1+master
	   * 
	   *A+B
	   *
	   *pull the required branch(master) to current branch (b1)
	 
	 
	 // lets say you have one project in git hub
	 
	 All the companies they will be having javaproject.
	 
	  take it to your local
	 
	 git clone <URL to the project>
	 can someone tell me.. If you have already having project how do you take it to eclipse
	 File --> import --> general / existing project into workspace -->select root directory and browse to your project --> Finish
	 
	 cmd -->  prompt
	 git bash --> git commands wil work
	 
	 git clone
	 Cloning is one time task --> you can clone only one default branch
	 when you dont have project --> 
	 ...
	 git branch <b1>
	 git branch <c1>
	 merge c1 and master --> master c1+master,
	 
	 b1 
	 latest master --> git pull origin master
	 ...
	 TRY TO ADDRESS QUESTIONS IN WORKSHOP. --> FEW OF YOUR QUESTIONS DEFNATLY WILL RESOLVE WHEN YOU REALLY WORK ON
